...
export async function backoffice(request: Request, env: BackofficeEnv, transport: typeof fetch = fetch): Promise<Response | null> {
  ...
  if (!env.ALOHA_AUTH || !env.ALOHA_AUTH_ORIGIN || !env.ALOHA_ADMIN_EMAILS || !env.ALOHA_EDITORIAL_CONFIG || (!env.GITHUB_CONTENT_TOKEN && !env.ALOHA_CONTENT)) {
  ...
  try {
    ...
    const auth = connexionAloha({ service: env.ALOHA_AUTH, origine: env.ALOHA_AUTH_ORIGIN, administrateurs: env.ALOHA_ADMIN_EMAILS.split(",") });
    ...
    const route = /^\/api\/editorial\/(fr|en)(?:\/([a-z0-9-]+))?$/.exec(path);
    ...
    if (env.ALOHA_CONTENT) {
      if (!await auth.identity(request)) return fail(401, "admin_required");
      const target = new URL(`/api/admin/theme-editorial/${site.id}/${route[1]}${route[2] ? `/${route[2]}` : ""}`, env.ALOHA_AUTH_ORIGIN);
    ...
    const editor = creerEditeur(site, {
      ...
      github: async (endpoint, init) => {
        ...
        if (endpoint !== "/graphql" && !endpoint.startsWith(`/repos/${site.depot}/contents/`)) throw new Error("Route GitHub invalide");
        return transport(new URL(endpoint, "https://api.github.com"), { ...init, redirect: "error", headers: {
