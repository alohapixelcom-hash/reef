...
import { ErreurGithub, lireArticles, lireReferences, enregistrerArticle } from "./github-editorial.ts";
...
export function creerEditeur(site: SiteEditorial, services: ServicesEditoriaux) {
  ...
  return async (req: Request, langue: Langue, slug?: string): Promise<Response> => {
    ...
    try {
      let input: { frontmatter?: Frontmatter; body?: string; sha?: string; unset?: string[] } = {};
      if (req.method === "POST") {
        ...
        if (!input || typeof input !== "object" || !input.frontmatter || Array.isArray(input.frontmatter) || typeof input.frontmatter !== "object") {
          return json({ error: "invalid_document" }, 400);
        }
      }
      const articles = await lireArticles(site, services, langue);
      ...
      if (req.method === "GET") {
        ...
        const refs = await lireReferences(site, services, langue);
      ...
      const saved = await enregistrerArticle(site, services, langue, slug, current?.extension ?? "md", stringifyPost(fm, body), current?.sha);
